...
## === Extract SNPs from the genotype files, for a single chromosome   



## +++ Call:
#
# called by extract_snps.sh 
#
...
if [ -s "${setfile}" ];then
  ...
  echo ""
  echo "  ERROR (extract_snps_chr.sh): Could not find the settings file \"${setfile}\"."
  echo ""
  exit 1  
fi
...
do
  if [ -z ${!var+x} ];then
    echo ""
    echo "  ERROR (extract_snps_chr.sh): mandatory variable $var is not defined."
    echo ""
    exit 1
  fi    
done
...
if [[ ! ${chrom} =~ ^[0-9]+$ ]];then    # autosomes only!       
  echo ""
  echo  "  ERROR (extract_snps_chr.sh): Chromosome name is not valid: " ${chrom} 
...
if [ "${gtype}" == "PGEN" ];then
  ...
  plink2 --pfile ${pfile_prefix} --extract ${sfile} --make-pgen --out ${outfile_prefix}
...
if [ "${gtype}" == "BED" ];then 
  ...
  outfile_prefix="${genofolder}/${new_genoid}_chr${chrom}"
  plink2 --bfile ${bfile_prefix} --extract ${sfile} --make-bed --out ${outfile_prefix}
  rm -f ${outfile_prefix}.log  
fi
